...
| Server | Purpose | When |
|--------|---------|------|
| **next-devtools** (nextjs_docs) | Next.js APIs, patterns, configuration | **PRIMARY** for any Next.js question |
...
| **context7** | Other library documentation | Non-Next.js libraries (React Query, Zod, etc.) |

**Note:** For Next.js, always use `next-devtools/nextjs_docs`. Use `context7` only for other libraries.
...
4. If not found → Build custom component following design tokens
...
| Server | Purpose | When |
|--------|---------|------|
| **vercel** | Deploy, manage projects, debug builds | Ready to ship or debugging deployments |
...
| **firecrawl** | Web scraping/research | Data extraction, competitive research |
| **browserbase** | Cloud browser automation | Testing across environments |
| **sentry** | Error tracking | Production debugging (needs config) |
| **supabase** | Database operations | When project_id is configured |
...
6. DEPLOY   → vercel (when ready to ship)
```
...
- `mcp_context7_resolve-library-id` → `mcp_context7_query-docs` — Non-Next.js library docs
...
- `mcp_next-devtools_nextjs_docs` — Next.js documentation (PRIMARY for Next.js)
...
- `mcp_vercel_deploy_to_vercel` — Deploy to Vercel
- `mcp_vercel_list_deployments` / `get_deployment` — View deployments
- `mcp_vercel_get_deployment_build_logs` — Debug failed builds
- `mcp_supabase_*` — Database operations (needs project_id config)
- `mcp_docker_*` — Container management (needs Docker on PATH)

### Web Research & Testing
- `mcp_firecrawl_firecrawl_scrape` — Scrape single URL
- `mcp_firecrawl_firecrawl_search` — Web search with scraping
- `mcp_firecrawl_firecrawl_extract` — Structured data extraction
- `mcp_cursor-browser-extension_browser_*` — Direct browser control
- `mcp_browserbase_browserbase_stagehand_*` — Cloud browser automation
- `mcp_serper_google_search` — Google search API

### Monitoring (Needs Config)
- `mcp_sentry_search_issues` — Find issues (needs API token fix)
- `mcp_sentry_get_issue_details` — Issue details
